...
/** Allocate a new vec with length len // set elements to NULL ??
*
*   actually allocated len+1 elems (i.e. one for length cell) plus OVER_MALLOC_BYTES to be safe  
*   vec[0] is set to len
*
*
*/
...
void wr_vec_zero(vec v) {
  ...
  int i;
  for (i=1; i<=v[0]; i++) v[i]=(gint)0; 
  
  //int bytenr;

  /* suppose v[0]=3: then we zero
     v[1],v[2],v[3] */
  /*     
  bytenr=v[0]*sizeof(gint);
  memset((void*)(v+1), 0, bytenr);
  */
}

/** Allocate a new cvec with length len // set elements to NULL, ??
* 
*  
*   actually allocated len+2 elems (i.e. 2 for length and freepos cells) 
*   vec[0] is set to len
*   vec[1] is set to 2 (first free pos)
*
*/
...
*
*  vec[0] is set to newly allocated len (at least i+1) 
*
*/
...
vec wr_vec_realloc(glb* g,vec v, int i) {
  ...
  if (i<=vlen) {
    ...
    //dprintf("Reallocing vec %d from %d to %d\n",(int)v,vlen,nlen);    
...
*
*  i is used as a raw array position: ie if i==0 then vec length is set
*
...
*
* i is used as a raw array position (i.e. if i==0 then length is set)
* If vec is not big enough, it is automatically reallocated and vec[0] set to new length
* iff free pos (cvec[1]) is less or equal to i, it is automatically set to i+i
*
*/
...
gptr wr_alloc_from_cvec(glb* g, cvec buf, gint gints) {
  ...
  if ((pos+gints)>=CVEC_LEN(buf)) {
    ...
    //printf("\n cpx12 CVEC_LEN(buf): %d, pos: %d, gints: %d\n",CVEC_LEN(buf),pos,gints);
    (g->alloc_err)=1;
    return NULL;    
  }  
