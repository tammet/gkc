...
char* make_auto_guide(glb* g, glb* kb_g, int guideparam) { 
  ...
  int probsize,eq,ueq,depth,length,size; //,bigratio;
  int iteration, queryok=1, qp1ok=1, qp2ok=1, qp3ok=1, sineok=1; // query preference strats default ok
  int tmp;
  ...
  */  
  ...
  size=3;
  if ((g->sin_clause_count)>50) sineok=1;
  else sineok=0;
