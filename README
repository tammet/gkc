...
instructions for running gkc, see the latest release v0.4:
...
See the `makefile` for optionally compiling without
`flex` and `bison`.

Compiling with autototools under Linux is not properly
maintained. However, if this is fixed, then to compile with 
autotools under Linux, rename `makefile`to something different and do

    ./Bootstrap
    ./configure
    make

which will first create `Makefile.am`
