...
static int wr_test_reasoner_otterparser(glb* g,int p) {
  ...
  err = wr_import_otter_file(g,"Rexamples/otter.txt",NULL,NULL,0);
  if(!err) {
...
static int wr_test_coretests(glb* g,int p) {
  ...
  if (!tmp) tmp=wr_test_eq(g,clvec,"p(1,10). p(1,10).",1,p);
  if (!tmp) tmp=wr_test_eq(g,clvec,"p(1,10). p(1,20).",0,p);
  if (!tmp) tmp=wr_test_eq(g,clvec,"p(1.234). p(1.234).",1,p);
  
  if (!tmp) tmp=wr_test_eq(g,clvec,"rp(1.23456). rp(1.23456).",1,p);
...
static int wr_test_eq(glb* g, cvec clvec, char* clstr, int expres, int p) {
  ...
  tmp=wr_import_otter_file(g,NULL,clstr,clvec,0);
  if (tmp) { 
  ...
  wr_print_term(g,t1);
  printf("\n");
  wr_print_record(g,clvec[2]);
  printf("term t2 \n");
  wr_print_term(g,t2);
  printf("\n");
  wr_print_record(g,clvec[3]);
  printf("\nprint ended\n");
  */
  res=1;
